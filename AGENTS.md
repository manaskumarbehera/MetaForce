...
- Keyboard shortcuts are **user-configurable** (options page recorder) and handled entirely in the content script: a capture-phase `window` keydown listener matches descriptors from `MF.getShortcuts(settings)` (`settings.shortcuts.{togglePanel,nextTab,prevTab}`, defaults Ctrl+Shift+M / Ctrl+Shift+. / Ctrl+Shift+,). Descriptors are `{ctrl,alt,shift,meta,code,key}` matched on the physical `event.code`; the pure helpers (`shortcutFromEvent`, `shortcutMatches`, `validateShortcut` — rejects browser-reserved combos like Ctrl/Cmd+T/N/W — `formatShortcut`) live in `mf_shared.js` and are unit-tested in `tests/Shortcuts.test.js`. The old fixed `chrome.commands` `toggle-panel` binding was removed (it would double-fire with the in-page listener); the content script still honors `{action:"togglePanel"}` messages for compat. Tab switching uses the `mfTabsApi` handle set by `buildTabStrip` (null when All Data is disabled).
- The trigger icon position is configurable (`settings.triggerPosition`: `middle-right` default / `top-right` / `bottom-right` legacy) via `applyTriggerPosition()` inline styles on `#mf-main`.
- **Every user-facing feature change must ship with in-product guidance**: bump `MF_WHATSNEW_VERSION` in `contentScript.js` and update the `mfMaybeShowWhatsNew` callout (shows once at the top of the panel until dismissed; flag `metaforceWhatsNew.seenVersion` in `chrome.storage.sync`). Its "Open settings" button messages `{action:"openOptionsPage"}` to the background.
